...
export default function Card({ status, pressHandler }) {
    ...
    const turnTo = (item) => {
        if (status) pressHandler(item.name);
    ...
    return (
        <View style={{width: '100%', flex: 1}}>
            <FlatList
                data={member}
                renderItem={({item}) => (
                    <TouchableOpacity style={styles.card} onPress={() => turnTo(item)}>
                        <View style={styles.cardContent}>
                            ...
                            {(item.status=='accepted') && status &&
                                <Ionicons name='checkmark-circle' size={40} color={'#19AC9F'} style={{position: 'absolute', right: 0}}/>
                            }
                            {(item.status=='checking') && status &&
                                <Ionicons name='ellipsis-horizontal-circle-sharp' size={40} color={'#F5C63E'} style={{position: 'absolute', right: 0}}/>
                            }
                            {(item.status=='denied') && status &&
                                <Ionicons name='close-circle' size={40} color={'#D34C5E'} style={{position: 'absolute', right: 0}}/>
                            }
                        </View>
                    </TouchableOpacity>
                )}
